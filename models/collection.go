...
type Collection struct {
	...
	NumberOfChapters     int64     `json:"number_of_chapters"`
	MostRecentUploadDate time.Time `json:"most_recent_upload_date"`
...
func (i *Collection) Create(conn *pgx.Conn, userID string) error {
	...
	fmt.Println(i.NumberOfChapters)
...
func (i *Collection) Delete(conn *pgx.Conn) error {
	_, err := conn.Exec(context.Background(), "DELETE FROM collection WHERE id=$1", i.ID)
